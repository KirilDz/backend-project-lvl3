...
test('main flow', async () => {
    ...
    const expectedUpdatedHTML = splitedHTML.slice(0, splitedHTML.length - 1)
        .join('\n');
    ...
    expect(linksForDownloading)
        .toEqual(expectedLinksForDownloading);
    expect(updatedLinksNames)
        .toEqual(expectedDownloadedFileNames);
    expect(updatedHtml)
        .toEqual(expectedUpdatedHTML);
    ...
    expect(expectedEditedHTMLFile)
        .toEqual(`${wroteHTMLdata}\n`);
    ...
    for (const [index, updatedLinkName] of Object.entries(updatedLinksNames)) {
        if (extname(updatedLinkName) !== '.png') {
            ...
            expect(wroteData)
                .toEqual(expectedFileData[+index]);
        }
    }
});
